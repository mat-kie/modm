...
namespace modm
{
	namespace platform
	{
		...
		public:
			...
			static void
			configureMac(ANResult negotiation)
			{
				uint32_t tmp;

				// TODO: this is most certainly redundant
				if (!negotiation.successful)
				{
					negotiation.mode = DuplexMode::Full;
					negotiation.speed = Speed::Speed100M;
				}

				MacConfiguration_t maccr{
					MacConfiguration::Ipv4ChecksumOffLoad |
					MacConfiguration::RetryDisable};
				maccr |= Speed_t(negotiation.speed);
				maccr |= DuplexMode_t(negotiation.mode);
		...
		private:
			...
			static bool
			writePhyRegister(uint32_t sid, uint32_t reg, uint32_t value);
			static bool
			readPhyRegister(uint32_t sid, uint32_t reg, uint32_t &value);
